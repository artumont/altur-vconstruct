...
def score(results):
    ...
    out["balanced_accuracy"] = (
        (out["tpr_synthetic"] + out["tnr_human"]) / 2 if n_syn and n_hum else None
    )
    with_conf = [r for r in answered if r["confidence"] is not None]
    if len(with_conf) == len(answered) and answered:
        probs = [
            r["confidence"] if r["is_synthetic"] else 1 - r["confidence"]
            for r in with_conf
        ]
...
def main():
    ...
    ap.add_argument(
        "--n", type=int, default=10, help="how many calls to send (0 = all)"
    )
    ...
    ap.add_argument(
        ...
        type=float,
        default=30.0,
        help="seconds per call, same as the judge",
    )
    ap.add_argument(
        "--out", help="write per-call results and the summary to this JSON file"
    )
    args = ap.parse_args()
    ...
    for i, row in enumerate(rows, 1):
        ...
        print(
            f"{i:3d}/{len(rows)} {row['anon_id']} truth={row['label']:9s} got={verdict:20s} {mark}{conf} {r['latency_s']:.2f}s",
            flush=True,
        )
        results.append(r)
    ...
    if args.out:
        json.dump(
            {"summary": summary, "results": results}, open(args.out, "w"), indent=1
        )
