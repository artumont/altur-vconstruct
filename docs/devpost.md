...
The answer is yes — and it can do it in under 200ms per call.
...
The system works on real telephony audio — 8 kHz, compressed, noisy — not clean lab recordings. It's designed to sit in-line with Altur's call infrastructure and score every call.

## How we built it

**SSL + MLP two-stage architecture:**

1. **WavLM-large** (Microsoft, 300M params) as a frozen self-supervised backbone it extracts 1024-dimensional acoustic embeddings from 4-second audio windows. We chose WavLM over wav2vec2 and XLS-R because it achieves the lowest EER (2.29%) on ASVspoof 2021, the standard anti-spoofing benchmark.

2. **3-layer MLP classifier** (1024 → 256 → 128 → 1) with BatchNorm and Dropout — trained on frozen embeddings. With only 282 training calls, we couldn't fine-tune the backbone without overfitting. The MLP trains in seconds, enabling rapid iteration.

3. **Isotonic calibration** — maps raw classifier scores to well-calibrated probabilities. The challenge uses `confidence` for tie-breaking, so arbitrary sigmoid outputs weren't acceptable.

4. **ONNX Runtime** — we export WavLM to ONNX at Docker build time, cutting inference latency ~2x on CPU versus PyTorch. The Docker image is lean: no transformers, no GPU libs.

**Key insight:** Instead of scoring all ~45 overlapping windows from a 3-minute call, we sample just 2 evenly-spaced windows. A human sounds human everywhere; a synthetic voice sounds synthetic everywhere. Two windows preserve the signal at a fraction of the compute.
...
- **Sub-200ms inference** on CPU for typical call lengths, with well-calibrated confidence scores
...
- **Production-ready deployment** — Docker multi-stage build, health endpoints, env-driven config, integration tests against real val audio
...
- **Latency constraints shape architecture.** The 2-window sampling trick was born from a scoring rule, not from ML theory, but it works because spoof signals are consistent across time.
