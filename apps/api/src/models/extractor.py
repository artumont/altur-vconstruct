...
def _default_onnx_path() -> Path:
    ...
    Returns:
        Existing WavLM ONNX checkpoint path.

    Raises:
        FileNotFoundError: If no WavLM ONNX checkpoint is found.
    """
...
class ONNXSSLEvaluator:
    ...
    def extract(self, waveform: torch.Tensor) -> torch.Tensor:
        ...
        Raises:
            ValueError: If waveform does not contain one 4-second window.
        """
    ...
    def extract_batch(self, waveforms: torch.Tensor) -> torch.Tensor:
        ...
        Raises:
            ValueError: If waveforms is not a 2-D batch.
        """
        if waveforms.dim() != 2:
        ...
        arr = (
            waveforms.numpy().astype(np.float32)
            if isinstance(waveforms, torch.Tensor)
            else waveforms.astype(np.float32)
        )
